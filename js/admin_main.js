...
window.onload = () => {
    ...
    const input = document.getElementById("woocommerce_sargapay-ada-plugin_mpk");
    if (input) {
...
function gen_extra_fields() {
    const input = document.getElementById('woocommerce_sargapay-ada-plugin_mpk');
    const ap_k_main = document.getElementById('woocommerce_sargapay-ada-plugin_blockfrost_key');
    const ap_k_test = document.getElementById('woocommerce_sargapay-ada-plugin_blockfrost_test_key');
    ...
    if (input) {
        ...
        button.addEventListener("click", function(event) {
            ...
            if ((input.value).trim().length > 0) {
                ...
                pay_address_mainet = generate_payment_address(xpub, index_value, 1, 0);
                //Generate Payment Address Testnet
                pay_address_testnet = generate_payment_address(xpub, index_value, 1, 1);
