...
export function generate_payment_address(xpub, lastindex, num_address, network_param) {
    const result = [];
    // Testnet = 0 Mainnet = 1
    const network = network_param == 0 ? Cardano.NetworkInfo.testnet().network_id() :
        Cardano.NetworkInfo.mainnet().network_id()
    try {
        // public key yoroi format
        var xpub_bech = Cardano.Bip32PublicKey.from_bytes(Buffer.from(xpub, 'hex')).to_bech32();
    } catch (error) {
