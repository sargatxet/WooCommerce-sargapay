    ...
    if (WC()->payment_gateways->payment_gateways()['sargapay-ada-plugin']->enabled) {
        ...
        if (wp_doing_ajax()) {
            if (isset($_POST['action_type'])) {
                $xpub = WC()->payment_gateways->payment_gateways()['sargapay-ada-plugin']->mpk;
                // 1=TESTNET 0=MAINNET
                $network = WC()->payment_gateways->payment_gateways()['sargapay-ada-plugin']->network == 1 ? 1 : 0;
                // wpdb call to check address index
                global $wpdb;
                $last_index_response = $wpdb->get_results("SELECT address_index FROM $table WHERE network=$network AND mpk='$xpub' ORDER BY id DESC LIMIT 1");
                if ($wpdb->last_error) {
                    //LOG Error
                    write_log($wpdb->last_error);
                } else {
                    if ($last_index_response === []) {
                        $last_index = 0;
                        $was_null = true;
                    } else {
                    ...
                    if ($action_type == "get_unused") {
                        ...
                        if ($network == 1) {
                            $response_query = $wpdb->get_var("SELECT COUNT(*) FROM $table WHERE status = 'unused' AND network = 1 AND mpk = '$xpub'");
                        } else {
                            $response_query = $wpdb->get_var("SELECT COUNT(*) FROM $table WHERE status = 'unused' AND network = 0 AND mpk = '$xpub'");
                        }
                        ...
                        if (!$addresses) {
                            ...
                            if (count($addresses) >= 1) {
                                if ($last_index != 0) {
                                    ...
                                    $first_address = $wpdb->get_var("SELECT COUNT(*) FROM $table WHERE network = $network AND mpk = '$xpub'");
                                    if ($first_address == 1) {
                                        $last_index = 1;
                                    }
                                }
                                ...
                                wp_send_json(__(' Addresses Generated and Saved in Database.', 'sargapay-plugin'));
                            }
                        }
                    }
                }
