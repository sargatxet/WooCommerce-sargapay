...
class Sargapay_ADA extends Sargapay_API
{
    ...
    {
        new Sargapay_DBs;
        SELF::$table = Sargapay_DBs::get_sargapay_ada_tables();
    }

    static function check_all_pendding_orders()
    {
        if (WC()->payment_gateways->payment_gateways()['sargapay-ada-plugin']->enabled) {
            global $wpdb;
            $orders = $wpdb->get_results("SELECT id, pay_address, order_id, order_amount, network FROM " . SELF::$table . " WHERE status = 'on-hold' OR status = 'validation'");
            if ($wpdb->last_error) {
                # LOG Error
                write_log($wpdb->last_error);
            } else if (count($orders) !== 0) {
                for ($i = 0; $i < count($orders); $i++) {
                    $network = $orders[$i]->network == 1 ? 1 : 0;
                    ...
                    # GET CONFIRMATIONS
                    $confirmation_obj = SELF::get_confirmations(
                    ...
                    if ($confirmation_obj->error === 0) {
                        ...
                        if ($confirmation_obj->confirmations > 0) {
                            if ($confirmation_obj->confirmations > WC()->payment_gateways->payment_gateways()['sargapay-ada-plugin']->confirmations) {
                            ...
                            $updated = $wpdb->update(SELF::$table, $data, $where);
                            if (false === $updated) {
                                // DB LOG ERROR.
                                write_log($wpdb->last_error);
                            }
                        ...
                        write_log("Error Call BLockForst API " . $confirmation_obj->error . $confirmation_obj->error_msg);
                    }
                }
            }
        }
    ...
    static function get_confirmations($payment_address, $payment_amount, $network, $order_was_made)
    {
        $confirmations = 0;
        ...
        if ($network == 1) {
            ...
            $api_key = WC()->payment_gateways->payment_gateways()['sargapay-ada-plugin']->blockfrost_test_key;
            $stake_key = substr($payment_address, 58, -6);
        } else {
            $url_network = 'https://cardano-mainnet.blockfrost.io/api/v0/';
            $api_key = WC()->payment_gateways->payment_gateways()['sargapay-ada-plugin']->blockfrost_key;
        ...
        $response_data = SELF::API_CALL($url, null, $header, false);
        ...
        if (isset($response_data->status_code)) {
            ...
            if ($has_ada) {
                ...
                $response_data = SELF::API_CALL($url, array(), $header, false);
                ...
                foreach ($response_data as $key) {
                    ...
                    if ($order_was_made <= $response_data->time) {
                        ...
                        // Loop for each input to find if the input came from the same wallet
            ...
            $result->error = -1;
            $result->error_msg = "API CALL EMPTY";
            return $result;
        }
    }
}
